...
export default function ContentManagementPage() {
  ...
  const filteredContent = useMemo(() => {
    return content
      ...
      .filter((c) => typeFilter !== 'all' ? c.type_id === typeFilter : true)
      .filter((c) => {
  ...
  const handleEdit = (contentId: string) => router.push(`/admin/content/edit/${contentId}`);

  const handleToggleStatus = async (contentId: string, currentIsDraft: boolean): Promise<void> => {
    const result = await toggleContentDraft(contentId, !currentIsDraft);
    if (result.success) {
      setContent((prev) => prev.map((c) => c.id === contentId ? { ...c, is_draft: !currentIsDraft } : c));
  ...
  const handleDragStart = (e: React.DragEvent, contentId: string) => {
    if (isFiltered) return;
    if (!selectedIds.has(contentId)) setSelectedIds(new Set([contentId]));
  ...
  const handleDrop = async (e: React.DragEvent, targetId: string) => {
    ...
    const result = await updateContentOrder(updatedItems.map((item) => ({ id: item.id, order_index: item.order_index })));
    if (!result.success) {
      showToast('error', result.error || 'Failed to update order');
      loadData();
    } else {
      showToast('success', idsToMove.size > 1 ? `Moved ${idsToMove.size} items` : 'Content order updated');
    }
  };
  ...
  const handleAutoScrollOnMove = (clientY: number) => {
    ...
    if (clientY < SCROLL_ZONE) window.scrollBy(0, -SCROLL_SPEED * (1 - clientY / SCROLL_ZONE));
    else if (clientY > vh - SCROLL_ZONE) window.scrollBy(0, SCROLL_SPEED * (1 - (vh - clientY) / SCROLL_ZONE));
  };
  ...
  const getRubberBandRect = (rb: RubberBand) => ({
    ...
    w: Math.abs(rb.currentX - rb.startX),
    h: Math.abs(rb.currentY - rb.startY),
  });
  ...
  const getItemsInRect = useCallback(
    (rect: { x: number; y: number; w: number; h: number }) => {
      ...
      for (const item of filteredContent) {
        ...
        if (
          cardRect.left < rect.x + rect.w && cardRect.right > rect.x &&
          cardRect.top < rect.y + rect.h && cardRect.bottom > rect.y
        ) ids.push(item.id);
      }
      return ids;
    },
  ...
  const handleGridMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    setRubberBand({ startX: e.clientX, startY: e.clientY, currentX: e.clientX, currentY: e.clientY, active: true });
    e.preventDefault();
  };
  ...
  const handleGridMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      setSelectedIds(new Set(getItemsInRect(getRubberBandRect(updated))));
    },
  ...
  const handleBulkPublish = async (isDraft: boolean) => {
    ...
    await Promise.all(
      ids.map((id) => {
        ...
        if (!item || item.is_draft === isDraft) return Promise.resolve();
        return supabase.from('content').update({ is_draft: isDraft }).eq('id', id);
      })
    );
    setContent((prev) => prev.map((c) => selectedIds.has(c.id) ? { ...c, is_draft: isDraft } : c));
    ...
    <>
      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => router.push('/admin/content/bulk')}
          ...
          {(['all', 'published', 'drafts'] as FilterType[]).map((f) => (
            <button
              ...
              className={`px-4 py-2 font-medium transition-colors ${
                activeFilter === f ? 'text-black border-b-2 border-black' : 'text-gray-500 hover:text-black'
              }`}
            >
              {f === 'all' && `All (${content.length})`}
              {f === 'published' && `Published (${publishedCount})`}
              {f === 'drafts' && `Drafts (${draftsCount})`}
            </button>
          ))}
        </div>
            ...
            >
              ...
              {videoTypes.length > 0 && (
                <optgroup label="Videos">
                  {videoTypes.map((ct) => <option key={ct.id} value={ct.id}>{ct.name}</option>)}
                </optgroup>
              )}
              {imageTypes.length > 0 && (
                <optgroup label="Images">
                  {imageTypes.map((ct) => <option key={ct.id} value={ct.id}>{ct.name}</option>)}
                </optgroup>
              )}
            </select>
          )}
        ...
        <div className="text-center py-20">
          <p className="text-gray-500 text-lg mb-4">
            {searchQuery || typeFilter !== 'all'
              ...
              : activeFilter === 'all' ? 'No content yet'
              : activeFilter === 'published' ? 'No published content'
              : 'No draft content'}
          </p>
      ...
      {selectedIds.size > 0 && !draggedId && (
        ...
        >
          <span className="text-sm font-semibold text-gray-200">{selectedIds.size} selected</span>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => { setDeleteModalOpen(false); setContentToDelete(null); }}
                disabled={isDeleting}
