...
export default function PortfolioManagementPage() {
  ...
  const filteredProjects = useMemo(() => {
    return projects
      ...
      .filter((p) => typeFilter !== 'all' ? p.type_id === typeFilter : true)
      .filter((p) => {
  ...
  const handleEdit = (projectId: string) => router.push(`/admin/portfolio/project/edit/${projectId}`);
  ...
  const handleToggleStatus = async (projectId: string, currentIsDraft: boolean): Promise<void> => {
    ...
    if (result.success) {
      setProjects((prev) => prev.map((p) => p.id === projectId ? { ...p, is_draft: !currentIsDraft } : p));
  ...
  const confirmDelete = async () => {
    ...
    if (result.success) {
      ...
      setSelectedIds((prev) => { const next = new Set(prev); next.delete(projectToDelete); return next; });
  ...
  const handleDragStart = (e: React.DragEvent, projectId: string) => {
    if (isFiltered) return;
    if (!selectedIds.has(projectId)) setSelectedIds(new Set([projectId]));
  ...
  const handleDrop = async (e: React.DragEvent, targetId: string) => {
    ...
    const result = await updateProjectsOrder(updatedItems.map((item) => ({ id: item.id, order_index: item.order_index })));
    if (!result.success) {
      showToast('error', result.error || 'Failed to update order');
      loadData();
    } else {
      showToast('success', idsToMove.size > 1 ? `Moved ${idsToMove.size} projects` : 'Project order updated');
    }
  };
  ...
  return (
    <>
      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        <button
          onClick={() => router.push('/admin/portfolio/project/create')}
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          <p className="text-gray-500 text-lg mb-4">
            {searchQuery || typeFilter !== 'all'
              ...
              : activeFilter === 'all' ? 'No projects yet'
              : activeFilter === 'published' ? 'No published projects'
              : 'No draft projects'}
          </p>
        ...
        <div ref={gridRef} className="relative select-none" onMouseDown={handleGridMouseDown}>
          <div className="grid grid-cols-1 gap-4" onDragOver={(e) => e.preventDefault()}>
