...
import { ArrowRight, Play, Camera, Film, Star, Mail, Zap, Heart } from 'lucide-react';

// --- Shared Components ---

const Section: React.FC<{ children?: React.ReactNode, className?: string }> = ({ children, className = "" }) => (
  <section className={`min-h-screen flex flex-col justify-center px-6 md:px-12 lg:px-24 py-24 relative overflow-hidden ${className}`}>
    {children}
  </section>
);
...
const PlaceholderImage = ({ label = "Image", className = "" }: { label?: string, className?: string }) => (
  <div className={`relative w-full h-full min-h-[400px] bg-surface-highlight rounded-4xl overflow-hidden flex items-center justify-center group ${className}`}>
...
const Hero = () => {
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center px-6 md:px-12 lg:px-24 pt-32 pb-20 overflow-hidden bg-surface">
      {/* Abstract Shapes */}
      <motion.div 
        style={{ y: y1 }}
        ...
        <div className="space-y-10 order-2 lg:order-1">
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl font-extrabold leading-[1.1] text-display mb-8">
              I Create <span className="text-accent relative inline-block">
                Worlds
                <svg className="absolute w-full h-3 -bottom-1 left-0 text-accent-pink" viewBox="0 0 100 10" preserveAspectRatio="none">
      ...
      <div className="max-w-5xl mx-auto px-6 text-center space-y-16 relative z-10">
        <FadeIn>
           <h2 className="text-4xl md:text-6xl font-extrabold leading-tight text-display">
             If you want to go viral,<br/>
             <span className="text-body opacity-40 decoration-wavy underline decoration-accent-pink">LEAVE THIS PAGE IMMEDIATELY.</span>
           </h2>
        </FadeIn>
...
const StoryBlock = ({ 
  ...
  return (
    <div className={`flex flex-col lg:flex-row gap-16 lg:gap-32 items-center py-24 ${reversed ? 'lg:flex-row-reverse' : ''}`}>
      <FadeIn className="flex-1 space-y-8">
        <div className={`w-20 h-2 bg-${highlightColor === 'text-accent' ? 'accent' : 'accent-pink'} rounded-full`} />
        <div className="text-2xl md:text-4xl leading-snug font-medium text-body">
          {text}
        </div>
      </FadeIn>
  ...
  return (
    <section className="px-6 md:px-12 lg:px-24 py-20 bg-surface">
      <div className="max-w-7xl mx-auto space-y-32">
        ...
        {/* Story Block 1 - Wrapped in a highlighted container for focus */}
        <div className="bg-surface-highlight rounded-[3rem] px-8 md:px-16">
            <StoryBlock 
            text={
                <>
                I used to chase numbers. But the more I created for others, the more I realized I was just adding to the <span className="text-display font-bold decoration-4 decoration-accent underline underline-offset-4">noise</span>. Then, the shift happened.
                </>
            }
        ...
        {/* High Impact Typography Section */}
        <div className="py-20 text-center">
            <FadeIn>
                ...
                <div className="flex justify-center my-8">
                     <div className="w-24 h-2 rounded-full bg-accent"></div>
                </div>
        ...
        <div className="bg-surface-highlight rounded-[3rem] px-8 md:px-16">
            ...
            text={
                <>
                By creating 30 TikToks a month, I would follow trends. By <span className="text-accent-pink font-bold">serving a vision</span>, I would generate them.
                </>
            }
            imageLabel="Vision"
        ...
        <div className="py-20 text-center">
           <FadeIn>
             ...
             <h3 className="text-5xl md:text-7xl font-extrabold text-display mb-12 leading-tight">
               Filmmaking became the <br/><span className="text-accent">ultimate means.</span>
             </h3>
    ...
    <Section className="bg-surface">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center max-w-7xl mx-auto w-full">
        <div className="space-y-16">
          <FadeIn>
            <h3 className="text-sm font-bold text-accent tracking-widest uppercase mb-6 bg-accent/10 inline-block px-4 py-2 rounded-full">The Process</h3>
          ...
          <div className="space-y-8">
            {steps.map((step, i) => (
              <FadeIn key={i} delay={i * 0.1}>
                <div className={`flex items-center gap-8 p-8 rounded-3xl transition-all ${step.highlight ? 'bg-display text-white shadow-xl scale-105' : 'bg-surface-highlight text-display hover:bg-white hover:shadow-lg'}`}>
                  <div className={`${step.highlight ? 'text-accent' : 'text-body'}`}>
                    {step.icon}
                  </div>
          ...
          <FadeIn>
            <p className="text-3xl font-extrabold text-accent-pink">
              Don't expect a camera; expect a set.
            </p>
          </FadeIn>
        </div>
  ...
  return (
    <section className="py-32 px-6 md:px-12 bg-white">
      <div className="max-w-6xl mx-auto text-center mb-24">
        <FadeIn>
          <h2 className="text-3xl md:text-5xl font-bold mb-8 text-body/40">
            "Just fluff. I don't believe you."
          </h2>
...
const Contact = () => {
  return (
    <Section className="bg-surface-highlight rounded-t-[5rem] mt-20">
          ...
          <form className="space-y-8 text-left">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-3">
                ...
                <input type="text" className="w-full bg-surface-highlight rounded-xl border-none p-5 text-display font-medium focus:ring-2 focus:ring-accent transition-all" placeholder="Your Name" />
              </div>
              <div className="space-y-3">
                <label className="block text-sm font-bold uppercase tracking-wider text-body">Email</label>
                <input type="email" className="w-full bg-surface-highlight rounded-xl border-none p-5 text-display font-medium focus:ring-2 focus:ring-accent transition-all" placeholder="email@example.com" />
              </div>
            </div>
            <div className="space-y-3">
              <label className="block text-sm font-bold uppercase tracking-wider text-body">The Vision</label>
              <textarea rows={4} className="w-full bg-surface-highlight rounded-xl border-none p-5 text-display font-medium focus:ring-2 focus:ring-accent transition-all" placeholder="Tell me about your world..."></textarea>
            </div>
...
const HomePage = () => {
  return (
    <div className="bg-surface text-display overflow-hidden selection:bg-accent selection:text-white">
      {/* Navigation */}
      <nav className="fixed top-0 left-0 w-full p-8 flex justify-between items-center z-50 bg-white/80 backdrop-blur-md border-b border-surface-highlight">
        <div className="text-2xl font-extrabold tracking-tight">PORTFOLIO.</div>
        <div className="hidden md:flex gap-10 text-sm font-bold uppercase tracking-wider text-body">
